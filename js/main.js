...
$(".selectElement_").change(function () {
    ...
    //console.log("!");
    //console.log($('input[name="mjerila_vrstauredjajaid"]').length);
    //console.log($('input[name="mjerila_vrstauredjajaid"]').val());
...
// Function to truncate text based on data-max-length attribute
function truncateText() {
    // Get all divs with the class 'truncate'
    const divs = document.querySelectorAll('.truncate');
    
    divs.forEach(div => {
      // Get the maximum length from the 'data-max-length' attribute
      const maxLength = parseInt(div.getAttribute('data-max-length'), 10);

      // Get the text content of the div
      const textContent = div.textContent;

      // If the text is longer than the specified max length, truncate it
      if (textContent.length > maxLength) {
        div.textContent = textContent.substring(0, maxLength) + '...';
      }
    });
  }

  // Call the function to truncate text when the page loads
